...
public class BinarySearchTree<T extends Comparable<T>> implements BinTree<T> {
...
BinTree<T> tree;
	...
	private T trouveMin(BinTree<T> tree) {
	...
	private BinTree<T> delete(T element, BinTree<T> tree) {
		...
		// Si l'élément est une feuille, donc vide, on retourne la feuille
		if (tree instanceof Feuille) {
			return tree;
		}
	...
	public ArrayList<T> bstToArrayList() {
		return bstToArrayList(tree);
	}
	
	private ArrayList<T> bstToArrayList(BinTree<T> tree) {
		
		// LIFO Data structure
		ArrayDeque<BinTree<T>> pile = new ArrayDeque<>();
		
		ArrayList<T> tabres = new ArrayList<>();
		
		while (true) {
			
			// Placer tous les arbres gauches dans la pile
			while (!tree.estVide()) {
				pile.push(tree);
				tree = tree.sag();
			}
			
			// Si on a épuisé tous les noeuds de la pile on peut sortir de la boucle
			if (pile.isEmpty()) break;
			
			// ajouter la valeur du le dernier noeud ajouté dans la pile à l'array
			tree = pile.pop();
			tabres.add(tree.racine());
			
			// considérer le sous arbre droit pour recommencer la boucle
			tree = tree.sad();
		}
		return tabres;
	}
